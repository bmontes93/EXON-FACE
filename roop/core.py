...
def decode_execution_providers(execution_providers: List[str]) -> List[str]:
    ...
    list_providers = [provider for provider, encoded_execution_provider in zip(available, encoded)
            if any(execution_provider in encoded_execution_provider for execution_provider in execution_providers)]
    
    try:
        for i in range(len(list_providers)):
            if list_providers[i] == 'CUDAExecutionProvider':
                # EXON Optimization: Tuned CUDA options for performance
                list_providers[i] = ('CUDAExecutionProvider', {
                    'device_id': roop.globals.cuda_device_id,
                    'cudnn_conv_algo_search': 'DEFAULT',
                    'arena_extend_strategy': 'kSameAsRequested',
                })
                torch.cuda.set_device(roop.globals.cuda_device_id)
            elif list_providers[i] == 'TensorrtExecutionProvider' and roop.globals.fp16_enabled:
                 list_providers[i] = ('TensorrtExecutionProvider', {'trt_fp16_enable': True, 'device_id': roop.globals.cuda_device_id})
    except:
        pass
...
def suggest_max_memory() -> int:
    if platform.system().lower() == 'darwin':
        return 8 # Increased for M-series
    return 32 # Increased for modern GPUs
...
def pre_check() -> bool:
    ...
    util.conditional_download(download_directory_path, ['https://github.com/yakhyo/face-parsing/releases/download/v0.0.1/resnet18.onnx'])
