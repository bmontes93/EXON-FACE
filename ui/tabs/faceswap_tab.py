...
def faceswap_tab():
    ...
    with gr.Tab("🎭 Face Swap"):
        ...
        with gr.Row(variant='panel'):
            ...
            bt_preset_ultra = gr.Button("💎 Ultra (Slow)", variant='primary')
            bt_preset_cinema = gr.Button("🎬 Cinema (Quality)", variant='primary')
            bt_preset_balanced = gr.Button("⚖️ Balanced", variant='secondary')
            bt_preset_fast = gr.Button("⚡ Fast (Speed)", variant='secondary')

        def apply_ultra():
            return gr.update(value="BiSeNet"), gr.update(value="Restoreformer++"), gr.update(value=True), gr.update(value="512px")

        def apply_cinema():
            # Cinema now prioritizes 256px with high-fidelity restoration
            return gr.update(value="BiSeNet"), gr.update(value="Restoreformer++"), gr.update(value=True), gr.update(value="256px")

        def apply_balanced():
            return gr.update(value="BiSeNet"), gr.update(value="GFPGAN"), gr.update(value=True), gr.update(value="256px")
            
        def apply_fast():
            return gr.update(value="DFL XSeg"), gr.update(value="None"), gr.update(value=True), gr.update(value="128px")
                    ...
                    with gr.Column(scale=2):
                        with gr.Accordion(label="Advanced Masking", open=False):
                            ...
                            selected_mask_engine = gr.Dropdown(
                                ["None", "Clip2Seg", "DFL XSeg", "BiSeNet"],
                                value="BiSeNet",
        ...
        with gr.Row(variant='panel'):
            ...
            with gr.Column(scale=1):
                num_swap_steps = gr.Slider(1, 5, value=2, step=1.0, label="Number of swapping steps", info="More steps may increase likeness")
            with gr.Column(scale=2):
                ui.globals.ui_selected_enhancer = gr.Dropdown(["None", "Codeformer", "DMDNet", "GFPGAN", "GPEN", "Restoreformer++"], value="Codeformer", label="Select Post-Processing", info="Enhances the improved face")
            with gr.Column(scale=2):
                 ui.globals.ui_blend_ratio = gr.Slider(0.0, 1.0, value=0.65, label="Enhancer Strength", info="Blend ratio: 1.0 = Full Enhancement, 0.0 = Original")

        with gr.Row(variant='panel'):
            with gr.Column(scale=1):
                max_face_distance = gr.Slider(0.01, 1.0, value=0.65, label="Max Face Similarity Threshold", info="Lower = Stricter matching (0.0 = identical)")
            with gr.Column(scale=1):
                ui.globals.ui_upscale = gr.Dropdown(["128px", "256px", "512px"], value="512px", label="Swap Resolution", interactive=True, info="Output resolution of the swapped face")
        ...
        with gr.Row(variant='panel'):
            ...
            with gr.Column(scale=1):
                with gr.Group():
                    ...
                    roop.globals.fp16_enabled = gr.Checkbox(label="Enable FP16 (Performance)", value=False, interactive=True)
    ...
    # Preset Events
    bt_preset_ultra.click(fn=apply_ultra, outputs=[selected_mask_engine, ui.globals.ui_selected_enhancer, roop.globals.fp16_enabled, ui.globals.ui_upscale])
    bt_preset_cinema.click(fn=apply_cinema, outputs=[selected_mask_engine, ui.globals.ui_selected_enhancer, roop.globals.fp16_enabled, ui.globals.ui_upscale])
    bt_preset_balanced.click(fn=apply_balanced, outputs=[selected_mask_engine, ui.globals.ui_selected_enhancer, roop.globals.fp16_enabled, ui.globals.ui_upscale])
    bt_preset_fast.click(fn=apply_fast, outputs=[selected_mask_engine, ui.globals.ui_selected_enhancer, roop.globals.fp16_enabled, ui.globals.ui_upscale])
...
def on_mask_engine_changed(mask_engine):
    if mask_engine == "Clip2Seg" or mask_engine == "BiSeNet":
...
def map_mask_engine(selected_mask_engine, clip_text):
    ...
    elif selected_mask_engine == "BiSeNet":
        mask_engine = "mask_bisenet"
    else:
        mask_engine = None
    return mask_engine
